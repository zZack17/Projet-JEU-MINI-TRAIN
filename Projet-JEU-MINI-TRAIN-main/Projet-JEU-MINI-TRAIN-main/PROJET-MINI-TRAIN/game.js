...
class Junction {
  ...
  draw(ctx) {
    ...
    // Contour lumineux en survol / animation
    // ctx => pinceau de canvas 
    if (this.hover || pulse > 0) {
      ctx.beginPath(); //commence un nouveau dessin solo
    ...
    if (seg) {
      // calculer la direction de la fleche 
...
class Segment {
  ...
  draw(ctx, highlight = false) {
    ...
    // Les traits qui sont sur les rails des p'tit detail
...
class Station {
  ...
  draw(ctx) {
    ...
    // Contour lumineux flash quand un train arrive dans la bonne gare ou pas
    if (f > 0) {
      ctx.beginPath();
...
// melange le tableau aleatoirement (pas d'ordre de couleur que se soit pour les trains ou gare)
...
function generateMap(diff) {
  ...
  // ── FACILE : 2 couleurs, 2 jonctions ── (representation de la premiere carte)
  ...
  // ── NORMAL : 3 couleurs, 3 jonctions ──(representation de la deuxieme carte)
  ...
  // ── DIFFICILE : 4 couleurs, 4 jonctions ──(representation de la troisieme carte)
  {
    //
    //    [S0]              [S2]
    //     |                 |
...
// apres avoir choisi quel niveau on souhaite faire cette fonction va construire la carte designer
class Game {
  constructor(diff) {
    this.diff = diff;
    ...
    this.W           = map.W; //largeur de la carte
    this.H           = map.H; //hauteur de la carte
    this.cfg         = map.cfg; //configuration de la carte 
  ...
  // ── Interactions ── (verifie si le click a toucher une jonction)
  ...
  // dès que mon curseur va passer sur une jonction il changera le curseur en forme de main
  onMouseMove(e) {
    const rect = this.canvas.getBoundingClientRect();
    const mx   = (e.clientX - rect.left) * (this.canvas.width  / rect.width);
    const my   = (e.clientY - rect.top)  * (this.canvas.height / rect.height);
  ...
  //creation nouveau trains avec nouvelle couleur associer au niveau 
  spawnTrain() {
    ...
    t.t = 0; //depart au tunnel 
    this.trains.push(t);
    this.trainsSent++;
    this.updateHUD(); // mets a jours les infos sur le temps , nb pts etc
  }
  ...
  //c'est le bouton play du moteur elle va continuer en boucle tant que gameOver n'est pas vrai
...
//c’est tout ce qui concerne l’affichage, les boutons, les messages, 
//les écrans, bref tout ce que le joueur voit et avec quoi il interagit
...
//si le joueur a rien choisi on part sur facile c'est la selection par defaut 
